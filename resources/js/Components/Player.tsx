...
export const Player = ({ allPlaylist, state, setState }) => {
  ...
  const displayVolumeBeforeWidth = () => {
    playerContainerRef.current.style.setProperty(
      '--volume-before-width',
      (volumeSliderRef.current.value / volumeSliderRef.current.max) * 100 + '%',
    );
  };
  ...
  const mutePlayer = () => {
    if (playerRef.current.muted) {
      ...
      playerContainerRef.current.style.setProperty(
        '--volume-before-width',
        (volumeSliderRef.current?.value / volumeSliderRef.current?.max) * 0 +
          '%',
      );
    }
  };

  function getRandomInt(min = 1, max: number) {
    min = Math.ceil(min);
    max = Math.floor(max);
  ...
  const addTrackToPlaylist = (playlistId, trackId) => {
    ...
    const playlist = allPlaylist.find(playlist => playlist.id === playlistId);
    const newPlaylist = {
      ...playlist,
        ...
        {allPlaylist.length > 0 &&
          allPlaylist.map(playlist => (
            <MenuItem
              onClick={() => {
                handleClose();
                addTrackToPlaylist(playlist.id, state.currentTrack);
              }}
              key={playlist.id}
          ...
          <div id="playerControls" className="flex flex-col w-full lg:w-min">
            ...
            <div id="seekSection" className="flex">
              ...
              <input
                ...
                type="range"
                ref={seekSliderRef}
                onChange={e => (seekSliderRef.current.value = e.target.value)}
